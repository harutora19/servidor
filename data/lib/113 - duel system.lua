...
function alertToPuchPokeball(p1, count, lose)
if not isCreature(p1) then return true end
   if not lose then
      doSendMsg(p1, "Você tem 1 minuto para puxar um pokemon ou perderar o duelo.")
	  addEvent(alertToPuchPokeball, duelTable.timeToPuch * 1000, p1, count, true)
   else
      if isInDuel(p1) and #getCreatureSummons(p1) <= 0 and getPokemonsCount(p1) == count then
...
function doRemoveCountPokemon(p1)
	...
	if getPokemonsCount(p1) == 0 then
	   ...
	   alertToPuchPokeball(p1, getPokemonsCount(p1), false)
    end	   
end
...
function doGiveUP(p1) 
	if not isCreature(p1) then return true end
	doSendAnimatedText(getThingPos(p1), "GIVE UP!", COLOR_BURN)
...
function doLoser(p1)
	if not isCreature(p1) then return true end
	doSendAnimatedText(getThingPos(p1), "LOSER!", COLOR_BURN)
	setPlayerStorageValue(p1, duelTable.loses, getPlayerStorageValue(p1, duelTable.loses) +1)
...
if getDistanceBetween(getThingPos(p2), getThingPos(p1)) > 8 then
...
if not isCreature(p1) or not isCreature(p2) then return true end
if not isInvitedDuel(p1, p2) and not (isInArray(tour_temp, p1) and isInArray(tour_temp, p2)) then return true end
if #getLivePokeballs(p1, getPlayerSlotItem(p1, 3).uid, true) < pokeballsCount then  
   doSendMsg(p1, "Você não tem a quantidade de pokemon exigida para este duelo.") 
   return true 
end
...
if getDistanceBetween(getThingPos(p2), getThingPos(p1)) > 8 then
   ...
   return true
	...
	doCreatureSetNoMove(p1, true)
	doCreatureSetNoMove(p2, true)
