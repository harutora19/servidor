...
Dz.LobbyPosition = {from = {x = 2813, y = 191, z = 7},to = {x = 2828, y = 209, z = 7}, tp = {x = 2820, y = 192, z = 7}}
Dz.Keys = {
  [DzBronze]  = {id = 'bronze',  name = "Bronze",  itemId = 18674},
  [DzSilver]  = {id = 'silver',  name = "Silver",  itemId = 18675},
  [DzGold]    = {id = 'gold',    name = "Gold",    itemId = 18676},
  [DzSpecial] = {id = 'special', name = "Special", itemId = 18435},
}
...
Dz.Chest = {
  [DzBronze]  = {itemId = 18433},
  [DzSilver]  = {itemId = 18433},
  [DzGold]    = {itemId = 18433},
  [DzSpecial] = {itemId = 18433},
}
    ...
    if DzTeam then
      ...
      for pos, name in pairs(DzTeam.members) do
        ...
        if isPlayer(player) then
		  -- members[#members+1] = {name = name, level = getPlayerLevel(player), outfit = getCreatureOutfit(player), clan = getPlayerClan(player)}
		  members[#members+1] = {name = name, level = getPlayerLevel(player), outfit = getCreatureOutfit(player)}
		end
	  end
...
function Dz.inviteToTeam(cid, name)
  ...
  if isInArray(DzTeam.members, name) then
    doPlayerPopupFYI(cid, "Este jogador já está no time.")
    return
  end
...
function Dz.play(cid, diff, mapId)
  ...
  if getPlayerItemCount(cid, Dz.Keys[Map.rarity].itemId) == 0 then
	...
	if getPlayerLevel(cid) <= Dz.Diff[diff].Level.min then
		doPlayerPopupFYI(cid, "Você está em um level muito abaixo da requerida.")
      return
	end
	if getPlayerLevel(cid) >= Dz.Diff[diff].Level.max then
		doPlayerPopupFYI(cid, "Você está em um level muito acima da requerida.")
      return
	end
  if state == DzStateTeam then
	...
	if #DzTeam.members <= Dz.Diff[diff].Level.min then
		doPlayerPopupFYI(cid, "Você está em um level muito abaixo da requerida.")
      return
	end
	if #DzTeam.members >= Dz.Diff[diff].Level.max then
		doPlayerPopupFYI(cid, "Você está em um level muito acima da requerida.")
      return
	end

	-- if getPlayerLevel(player) <= Dz.Diff[Map.rarity].Level.min then return {name = name, msg = "Algum membro do grupo não possuo level suficiente."} end 
	-- if getPlayerLevel(player) >= Dz.Diff[Map.rarity].Level.max then return {name = name, msg = "Algum membro do grupo não possuo level suficiente."} end 
	-- if getPlayerLevel(player) >= Dz.Diff[Map.rarity].Level.max then return doPlayerPopupFYI(player, (isPlayer(player) and getCreatureName(player) or "Um jogador").." não possui level suficiente.") end
    ...
    -- elseif ret.msg == DzNotHaveLevel then
      -- return doPlayerPopupFYI(cid, (isPlayer(player) and getCreatureName(player) or "Um jogador").." não possui level suficiente.")
    end
  else
    if not isInRange(getThingPos(cid), Dz.LobbyPosition.from, Dz.LobbyPosition.to) then
      doPlayerPopupFYI(cid, "Você não na sala de espera.")
	  Dz.sendClose(cid)
	  return
    end
    local roomId = Dz.getAvailableRoom(Map)
...
function Dz.startMap(members, diff, mapId, roomId)
  ...
  for _, pokemon in ipairs(Room.pokemons) do
	...
	-- local pid = doCreateMonsterNick(pid, pokemon.name, retireShinyName(pokemon.name), pokemon.pos, false)		  
...
function Dz.doPlayerLeave(cid)
  ...
  elseif value.state == DzStateReward then
	...
	-- if #DzTeam.members >= Dz.Diff[diff].Level.min or Dz.Diff[diff].Level.max >= #DzTeam.members then
	-- Dz.Diff[DzBeginner].Level = {min=80, max=119}
		-- doPlayerPopupFYI(cid, (isPlayer(player) and getCreatureName(player) or "Um jogador").." não possui level suficiente.")
		-- return 
	-- end
...
function Dz.AreMembersPrepared(Map, members, remove)
  for pos, name in pairs(members) do
	...
	-- if getPlayerLevel(player) <= Dz.Diff[Map.rarity].Level.min then return {name = name, msg = DzNotHaveLevel} end
	-- if getPlayerLevel(player) >= Dz.Diff[Map.rarity].Level.min then return {name = name, msg = DzNotHaveLevel} end
  end
...
-------------------------
-------- Dentro de cada action que use potion ou revive, usar essa verificaÃ§Ã£o, sendo que dentro da tabela, vai variar revive/potion
...
function Dz.onUseMedicament(cid, params)
  ...
  if params.potion then
    if value.potions == 0 then
	  doPlayerPopupFYI(cid, "Você já utilizou todas as poções disponíveis.")
	  return false
	end
  ...
  elseif params.revives then
    if value.revives == 0 then
	  doPlayerPopupFYI(cid, "Você já utilizou todos os revives disponíveis.")
	  return false
	end
...
function Dz.teleportToPrize(diff, mapId, roomId)
  ...
  for _, name in pairs(Room.variable.members) do
    local player = getPlayerByName(name)
    Dz.addToRanking(diff, mapId, {name = name, outfit = getCreatureOutfit(player), level = getPlayerLevel(player), time = os.time()-Room.variable.time})
    doSendPlayerExtendedOpcode(player, Dz.Opcode, json.encode({protocol = "closeinformation"}))
    ...
    if not prizeRoomId then
      ...
      for id, pos in ipairs(PrizeRoom.corpse) do
        ...
        if math.random(1, id == 1 and 50 or 70) < 50 then
          ...
          doItemSetAttribute(corpse, "corpseowner", getPlayerByName(player))
		  doItemSetAttribute(corpse, "pokeName", "fainted " .. pokemon)
