...
function adjustWildPoke(cid, rate)
    if not isCreature(cid) then return true end
    if isSummon(cid) then return true end
    local r = rate or 1

    if isNPCSummon(cid) then return true end -- summmon npca
    ...
    setPlayerStorageValue(cid, 1001, pokes[nick].attack * (1 + r / 10))
    setPlayerStorageValue(cid, 1002, pokes[nick].defense * (1 + r / 10))
    setPlayerStorageValue(cid, 1003, pokes[nick].agility)
    setPlayerStorageValue(cid, 1004, pokes[nick].vitality * level * r)
    setPlayerStorageValue(cid, 1005, pokes[nick].specialattack * (1 + r / 10))

    doRegainSpeed(cid)	     --alterado!

    local pokeLifeMax = pokes[nick].life + (pokes[nick].vitality * pokes[nick].wildLvl * 4.25)
    -- if r > 1 then
        -- pokeLifeMax = pokeLifeMax * r * 2
    -- end

    -- if string.find(nick, "Shiny") then
        -- pokeLifeMax = pokeLifeMax * 2
    -- end
...
function adjustStatus(pk, item, health, dittoLife, conditions, isPassiva)
    ...
    local combineTable = {
        ...
        ["X-Vitality"] = {"X-Critical", "X-Defense", "X-Attack", "X-Return", "X-Block"},
    }
    -- Defense --
    ...
    if isInArea(getThingPos(getCreatureMaster(pk)), {x=127, y=820, z=6}, {x=135, y=828, z=6}) then
        if masterLevel < 249 then
            masterLevel = 249
        elseif masterLevel > 249 and masterLevel < 349 then
            masterLevel = 349
        elseif masterLevel > 349 and masterLevel < 500 then
            masterLevel = 500
        end
    end

    --if isGod(getCreatureMaster(pk)) then doSendMsg(getCreatureMaster(pk), "masterLevel: ".. masterLevel) end


	if string.find(nick, "Shiny ") then
		setPlayerStorageValue(pk, 1001, (pokes[nick].attack * bonusShiny) * heldBonusAttack)
		setPlayerStorageValue(pk, 1002, (pokes[nick].defense * bonusShiny) * heldBonusDefense)
		setPlayerStorageValue(pk, 1003, (pokes[nick].agility * bonusShiny) + heldBonusSpeed)
		setPlayerStorageValue(pk, 1004, (pokes[nick].vitality * bonusShiny) * (masterLevel + getPokemonBoost(pk) + heldBonusBoost))
		setPlayerStorageValue(pk, 1005, (pokes[nick].specialattack * bonusShiny) * heldBonusAttack)
	elseif string.find(nick, "Mega ") then
		setPlayerStorageValue(pk, 1001, (pokes[nick].attack * bonusMega) * heldBonusAttack)
		setPlayerStorageValue(pk, 1002, (pokes[nick].defense * bonusMega) * heldBonusDefense)
		setPlayerStorageValue(pk, 1003, (pokes[nick].agility * bonusMega) + heldBonusSpeed)
		setPlayerStorageValue(pk, 1004, (pokes[nick].vitality * bonusMega) * (masterLevel + getPokemonBoost(pk) + heldBonusBoost))
		setPlayerStorageValue(pk, 1005, (pokes[nick].specialattack * bonusMega) * heldBonusAttack)
	else
		setPlayerStorageValue(pk, 1001, pokes[nick].attack * heldBonusAttack)
		...
		setPlayerStorageValue(pk, 1004, pokes[nick].vitality * (masterLevel + getPokemonBoost(pk) + heldBonusBoost))
		setPlayerStorageValue(pk, 1005, pokes[nick].specialattack * heldBonusAttack)
	end

	local life_boost = 40 * (masterLevel + getPokemonBoost(pk) + heldBonusBoost)
		
	local pokeLifeMax = pokes[nick].life
	local boost = getPokemonBoost(pk) + heldBonusBoost
	
	-- if string.find(nick, "Shiny ") then
		-- local life = ((pokes[nick].vitality * (masterLevel + boost)) * 12.5) * (heldBonusVitality / 100 + 1) * bonusMega
	-- elseif string.find(nick, "Mega ") then
		-- local life = ((pokes[nick].vitality * (masterLevel + boost)) * 12.5) * (heldBonusVitality / 100 + 1) * bonusShiny
	-- else
		local life = ((pokes[nick].vitality * (masterLevel + boost)) * 12.5) * (heldBonusVitality / 100 + 1)
	-- end
	
	-- print(life)

    --if isGod(getCreatureMaster(pk)) then doSendMsg(getCreatureMaster(pk), "Boost:".. boost .. "Life: ".. life) end

	if string.find(nick, "Shiny ") then
		setCreatureMaxHealth(pk, life * 1.20)
	elseif string.find(nick, "Mega ") then
		setCreatureMaxHealth(pk, life * 1.40)
	else
	...
	local aura = getItemAttribute(item, "particle")
	if aura then
		addEvent(doSendCreatureEffect, 100, pk, 1)
	end
...
function doEvolvePokemon(cid, item2, theevo, stone1, stone2, count)

    if not isCreature(cid) then return true end

    if not pokes[theevo] or not pokes[theevo].attack then
	...
	setMonsterLevel(pk, getBallPokemonLevel(pokeball.uid))
    ...
    local c = count or 1
    if stone1 then doPlayerRemoveItem(cid, stone1, c) end
    if stone2 then doPlayerRemoveItem(cid, stone2, c) end
