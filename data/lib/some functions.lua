...
function addPokeToPlayer(cid, pokemon, boost, ball, unique, mega)             --alterado v1.9 \/ peguem ele todo...
...
lookClans2 = {
["volcanic"] = {"a Volcanic Spark", "a Volcanic Flame","a Volcanic Firetamer","a Volcanic Pyromancer","a Volcanic Master"},
["seavell"] = {"a Seavell Drop", "a Seavell Icelake","a Seavell Waterfall","a Seavell Frost","a Seavell Master"},
["orebound"] = {"an Orebound Sand", "an Orebound Rock","an Orebound Solid","an Orebound Hardskin","an Orebound Hero"},
["wingeon"] = {"a Wingeon Cloud", "a Wingeon Wind","a Wingeon Sky","a Wingeon Falcon","a Wingeon Dragon"},
["malefic"] = {"a Malefic Troublemaker", "a Malefic Venomancer","a Malefic Spectre","a Malefic Nightwalker","a Malefic Lord"},
["gardestrike"] = {"a Gardestrike Fist", "a Gardestrike Tamer","a Gardestrike Fighter","a Gardestrike DeathHand","a Gardestrike Champion"},
["psycraft"] = {"a Psycraft Mind", "a Psycraft Brain","a Psycraft Scholar","a Psycraft Telepath","a Psycraft Medium"},
["naturia"] = {"a Naturia Seed", "a Naturia Sprout","a Naturia Webhead","a Naturia Woodtrunk","a Naturia Keeper"},
["raibolt"] = {"a Raibolt Shock", "a Raibolt Watt","a Raibolt Electrician","a Raibolt Overcharged","a Raibolt Legend"},
["ironhard"] = {"an Ironhard Smither", "an Ironhard Forge","an Ironhard Hammer","an Ironhard Metal","an Ironhard Titan"},
}
...
if (getPlayerClan(thing) ~= "Pokemon Trainer" and (not getPlayerClanRank(thing) or (getPlayerClanRank(thing) < 1 or getPlayerClanRank(thing) > 5)) and getPlayerLevel(thing) > 120) then
	setPlayerClanRank(thing, 5)
end
local rank = (getPlayerStorageValue(thing, 86228) <= 0) and "a Pokemon Trainer" or lookClans2[getPlayerClan(thing)][getPlayerClanRank(thing)]
...
local myart = thing == cid and "You've had" or (getPlayerSex(thing) == 0 and "She had" or "He had")
...
if isPremium(thing) then
	table.insert(str, " "..art.." a VIP Player.")
end
    ...
    if ballSlot.uid ~= 0 then
       for a, b in pairs (pokeballs) do
           if ballSlot.itemid == b.on or ballSlot.itemid == b.use then
              if duel and ((getPlayerLevel(cid) >= (pokes[getItemAttribute(ballSlot.uid, "poke")].level + getPokeballBoost(ballSlot))) or isReborned(cid)) then
...
if getPlayerStorageValue(cid, 17230) then
	if os.time() < getPlayerStorageValue(cid, 17231) then
		doSetCreatureOutfit(cid, {lookType = getPlayerStorageValue(cid, 17230)}, -1)
	end
end
...
function doRemoveTargetsOnMe(cid)
	local mypos = getThingPos(cid)
	doTeleportThing(cid, {x = 196, y= 793, z = 6}, true)
	doTeleportThing(cid, mypos, true)
end
...
if agi > 0 then
   setPlayerStorageValue(cid, 1003, getPokemonBaseSpeed(cid))
   -- if getCreatureSpeed(cid) ~= 0 then
      doChangeSpeed(cid, -agi)
   -- end                                              --alterado v1.5  functions arrumadas...
end
end

function getPokemonBaseSpeed(cid)
if not isCreature(cid) then return false end
...
-- function raiseStatusCounter(cid, off, def, agi, time)
	-- if not isCreature(cid) then return true end
	-- sendOpcodeStatusInfo(cid)
	-- if time == -1 then return true end
	-- if off > 0 then
		-- setPlayerStorageValue(cid, 13898, time)
	-- elseif def > 0 then
		-- setPlayerStorageValue(cid, 13899, time)
	-- elseif agi > 0 then
		-- setPlayerStorageValue(cid, 13900, time)
	-- end
	-- return addEvent(raiseStatusCounter,1000,cid, off, def, agi, time-1)
-- end
...
local A = getAttack(cid)
local B = getDefense(cid)
local C = getPokemonBaseSpeed(cid)
...
-- raiseStatusCounter(cid, off, def, agi, time)
...
function hasPokemon(cid)
	if not isCreature(cid) then return false end
	if getPlayerCap(cid) <= 0 then return false end
...
function doCorrectString(str)
	...
	local ret = table.concat(final, str:find("-") and "" or (name[2] and " " or ""))
	return ret
end   
...
if not isInArray({5, 6}, getPlayerGroupId(cid)) then
   ...
   local low = {}
   local lowPokes = {"Rattata", "Caterpie", "Weedle", "Oddish", "Pidgey", "Paras", "Poliwag", "Bellsprout", "Magikarp", "Hoppip", "Sunkern", "Charizard"}
   if #balls >= 1 then
...
function getBallCD(item, move)
	local str = getItemAttribute(item, "cd"..move) 
end
