local poke = {"Shiny Bronzong", "Gardevoir", "Shiny Gardevoir", "Espeon", "Shiny Xatu", "Jynx", "Shiny Jynx", "Xatu", "Natu", "Exeggutor", "Slowking", "Slowbro", "Shiny Mr. Mime", 'Mew', 'Mewtwo', 'Abra', 'Kadabra', 'Alakazam', 'Drowzee', 'Hypno', 'Mr. Mime', 'Porygon', 'Shiny Abra', 'Shiny Alakazam', 
'Shiny Hypno', 'Porygon2', "Umbreon", "Shiny Umbreon"}  --alterado v1.9
...
[9] = {name = "pewter", id = 9},      
[10] = {name = "golden", id = 10},       
[11] = {name = "phenac", id = 11, level = 250, pos = {x=2152,y=217,z=7}},
[12] = {name = "agate", id = 12, level = 250, pos = {x=2122,y=386,z=5}},
-- [13] = {name = "ascorbia", id = 13},
[13] = {name = "Outland North", id = 14, level = 150, pos = {x=2812,y=2693,z=6}},
[14] = {name = "Outland East", id = 15, level = 150, pos = {x=3169,y=2915,z=6}},
[15] = {name = "Outland South", id = 16, level = 150, pos = {x=2823,y=3075,z=6}},
}
...
function onSay(cid, words, param)
	...
	if myplace == "phenac" then
		if getPlayerLevel(cid) < places[11].level then
			doPlayerSendCancel(cid, "You don't have enough level.")
			return true
		end
		telepos = places[11].pos
	end
	
	if myplace == "agate" then
		if getPlayerLevel(cid) < places[12].level then
			doPlayerSendCancel(cid, "You don't have enough level.")
			return true
		end
		telepos = places[12].pos
	end

	if myplace == "Outland North" then
		if getPlayerLevel(cid) < places[13].level then
			doPlayerSendCancel(cid, "You don't have enough level.")
			return true
		end
		telepos = places[13].pos
	end

	if myplace == "Outland East" then
		if getPlayerLevel(cid) < places[14].level then
			doPlayerSendCancel(cid, "You don't have enough level.")
			return true
		end
		telepos = places[14].pos
	end

	if myplace == "Outland South" then
		if getPlayerLevel(cid) < places[15].level then
			doPlayerSendCancel(cid, "You don't have enough level.")
			return true
		end
		telepos = places[15].pos
	end
