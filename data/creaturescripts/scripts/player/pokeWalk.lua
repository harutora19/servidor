function onMove(cid, fromPosition, toPosition)
     if isPlayer(cid) then
		if getCreatureSkullType(cid) == SKULL_WHITE then
			...
			if getDistanceBetween(getThingPos(p2), toPosition) > 8 then
				  doWin(p2)
				  doGiveUP(cid)
     ...
     if isWild(cid) then
		...
		if isConditionImune(cid) and getClosestTarget(cid) ~= false and getCreatureTarget(cid) ~= getClosestTarget(cid) then
			local new = doReloadPokemon(cid)
			doSetPokemonAgressiveToPlayer(new, getClosestTarget(cid))
			--print(getCreatureName(cid).." should attack ".. getClosestTarget(cid))
		end
